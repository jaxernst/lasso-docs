...
`eth_sendRawTransaction` is routed with one upstream dispatch. `eth_sendTransaction` and the node-local methods listed in [Supported methods](/api/supported-methods) are rejected.
