...
The connection can close when you disconnect, fail to answer heartbeats, lose profile access, encounter a network interruption, or the server restarts. Subscription continuity exhaustion or delivery backpressure can also close the connection.
...
Code 1013 indicates retryable delivery backpressure or resource exhaustion. Slow client processing, subscription volume, or server-wide delivery limits can trigger it; the close reason alone does not identify the cause. Reconnect with capped backoff, reconcile missed events, and check whether your client can keep up with its subscriptions.
