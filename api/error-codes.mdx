...
`message`, and optional `data`. An error's numeric code alone does not prove
...
An upstream may return a valid JSON-RPC error with HTTP 2xx, 4xx, or 5xx.
Core preserves a structurally valid, request-correlated provider error rather
than replacing its code or wording with a generic Lasso message. Gateway text,
HTML, and malformed envelopes are transport failures.
