...
Compatible clients subscribing to `newHeads` can share an upstream
subscription. On provider failure, Lasso performs bounded HTTP replay and
either resumes on a replacement or terminates the affected clients explicitly.
