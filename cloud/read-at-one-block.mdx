...
description: Use one block hash for related state reads.
---
...
- Lasso sends block-hash state reads only to archival providers, because a hash's
  age is unknown before dispatch. On a chain or profile without one, these reads
  fail instead of reaching a provider that may lack the state.
...
[Block regression protection](/cloud/block-continuity) separately protects successive
latest-block requests. [Routing metadata](/cloud/observability#opt-in-metadata)
is optional.
