...
description: Read balances, transaction counts and contract state at the same block with standard JSON-RPC.
---
...
Lasso server, enable [Block regression protection](/cloud/block-continuity).
Its server-switch and restart recovery is best effort. It does not change
explicit historical targets or pin ordinary `latest` state calls automatically.

## Optional: inspect routing metadata

Add `?include_meta=headers` to see request IDs, providers and retries while
preserving the JSON-RPC body. See [routing metadata](/cloud/observability#opt-in-metadata).
