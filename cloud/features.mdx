...
- [Block regression protection](/cloud/block-continuity) prevents sequential
...
- [Read at one block](/cloud/read-at-one-block) uses a standard block-hash selector
...
- Anonymous keys can be created without signup and later claimed by an account
  without changing the RPC endpoint. Funding depends on the deployment.
...
- Stripe-backed account billing is separate from anonymous-key funding and
  paid RPC sessions. Production currently disables MPP funding and sessions
  (checked September 20, 2026). The implemented legacy Tempo session protocol
  is not a promise of compatibility with the current Tempo CLI. Check the live
  [service manifest](https://lasso.sh/agent.json) and
  [deployment guide](https://lasso.sh/SKILL.md) before attempting payment.
- Fetch the live [pricing catalog](https://lasso.sh/api/v1/agent/pricing) for
