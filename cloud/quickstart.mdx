...
within the user’s existing resource and spending authority. The preview
[agent workflow](/cloud/agent-flows) provides separate management credentials and
recoverable CLI onboarding when enabled.
...
Give the short-lived `claim_url` to the user. Claiming keeps the RPC endpoint and premium scope, moves purchased credit to the
