...
| `latency-weighted` | Samples a weighted order from recent successful latency after reliability qualification. | You want a performance bias without always selecting one provider. |
| `fastest` | Ranks measured latency by provider, method, and transport. | Latency matters enough to accept traffic concentration. |
| `priority` | Uses configured provider order within the same health tier. | You want an explicit first preference. |
