...
Turn **Block regression protection** **On** in a chain's settings to protect
sequential block choices on each running server. Progress is shared automatically
across the fleet, with best-effort recovery after a server switch or restart.
**Off** is the default. Choose a block once and reuse its hash through the complete
query; ordinary `latest` state calls are not automatically grouped. See
[Block regression protection](/cloud/block-continuity)
