...
description: Keep later block choices from going backwards across regions.
---

Keep each query at one block, and keep later block choices from going backwards.

Enable **Block continuity** for a chain in your custom profile. Then choose a
...
The guarantee is shared by all callers, API keys, and serving regions on the
...
The continuity floor survives service restarts. Disabling the setting suspends
