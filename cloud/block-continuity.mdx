...
description: Choose local or global protection for later block choices.
---

Keep each query at one block, and keep later block choices from going backwards.

Choose **Local** under **Block continuity** in your custom profile's chain
settings to protect sequential block choices on each running instance, with
best-effort recovery after moving to another instance. **Global** offers the
stronger coordinated fleet-wide contract when configured. **Off** is the default.
