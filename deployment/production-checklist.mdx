...
Core v0.4.5 exposes `GET /api/health` for application liveness. Configure your
orchestrator or load balancer to poll this endpoint for process availability.

The health endpoint confirms that the application is running and reports cluster
topology. It does not make live upstream RPC requests or prove route readiness;
pair it with provider and routing monitoring and an upstream-backed RPC check.
...
- [ ] Console log drain configured; add a JSON formatter at deployment level if your collector requires JSON
