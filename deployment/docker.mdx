...
The example below installs the verified v0.4.5 container. See [release evidence](/releases-and-availability)
...
in `container-release.json`. See [Releasing](https://github.com/jaxernst/lasso-rpc/blob/v0.4.5/docs/RELEASING.md) for the verification
...
need matching filenames and frontmatter slugs. See [Configuration](https://github.com/jaxernst/lasso-rpc/blob/v0.4.5/docs/CONFIGURATION.md)
...
In v0.4.5, new profiles can reuse connected WebSocket upstreams after reload.
If you remain on v0.3.4, recreate the container after adding profiles to avoid
unavailable subscriptions. For new profiles and YAML-only edits in v0.4.5, reload
the running node:
