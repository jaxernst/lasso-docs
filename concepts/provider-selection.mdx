...
description: 'Eligibility, ranking and live admission in Core v0.4.3'
---

**Lasso RPC Core v0.4.3** selects upstream channels from the requested file profile
...
[file schema](https://github.com/jaxernst/lasso-rpc/blob/v0.4.3/lib/lasso/config/file_schema.ex)
and [configuration reference](https://github.com/jaxernst/lasso-rpc/blob/v0.4.3/docs/CONFIGURATION.md#provider-capabilities).

## Strategy ranking

[Routing strategies](/concepts/routing-strategies) define channel order:
random shuffle, recent reliability-qualified latency, relative latency weights,
...
Open circuits are excluded. Strategy order remains intact within each tier.
