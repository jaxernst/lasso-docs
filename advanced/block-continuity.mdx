...
description: Protect latest-block responses, with automatic sharing across connected RPC Core instances.
---
...
required for this mode. Configure [clustering](/deployment/clustering) to connect
...
The separate [Read at one block](/advanced/read-at-one-block) guide shows this standard
...
See [protection metadata](/observability/request-metadata#block-protection-metadata) for the
...
follow the [operating guide](/deployment/block-continuity).
