...
- Lasso sends block-hash state reads only to providers declared archival, because
  a hash's age is unknown before dispatch. A chain without one rejects them with
  [`archive_required`](/api/error-codes#core-routing-exhaustion).
