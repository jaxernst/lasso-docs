...
description: Build a complete logical query over standard Ethereum RPC.
---
...
Configure `head_policy: local` for protection on each running instance with
...
[Block regression protection](/advanced/block-continuity) for the scope and freshness bound.
...
legitimately have no `head_policy` object. See [routing evidence](/observability/request-metadata).
...
The optional [logical-read example](https://github.com/jaxernst/lasso-rpc/tree/main/examples/logical-read) uses
