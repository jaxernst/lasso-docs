...
Provider height monitoring informs routing and lag observations. It does not by
itself establish a monotonic cross-region block choice or prove chain consensus.
Use [Block regression protection](/advanced/block-continuity) for its protection
and recovery contracts, and
[hash-pinned reads](/advanced/read-at-one-block) for one-block queries.
...
When the WebSocket subscription is active, Core keeps HTTP polling as a
cross-check but uses three times the configured interval. It restores the
normal interval when the subscription becomes unavailable.
...
ProbeCoordinator checks work on a 200 ms scheduling tick. Each upstream's
effective probe cadence comes from the shortest `monitoring.probe_interval_ms`
among the profiles that reference it, or the chain's block-time default when
none applies. Failure backoff can delay a probe further.
...
Set `monitoring.probe_interval_ms` in a chain's profile configuration when you
need a different probe cadence. Profiles sharing an upstream use the shortest
configured interval. The 200 ms tick is a scheduler, not a request to probe
every upstream five times per second.
