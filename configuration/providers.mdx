...
In RPC Core v0.4.1, `subscribe_new_heads` controls automatic block tracking and client `newHeads` eligibility. Set it to `true` for providers you intend to use for those subscriptions. A configured WebSocket URL alone does not enable this opt-in. See [Profile configuration](/configuration/profiles).
