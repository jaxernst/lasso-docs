...
In RPC Core v0.4.5, `subscribe_new_heads` controls automatic block tracking and client `newHeads` eligibility. Providers inherit the chain setting, which defaults to `true`; set a provider override to `false` when that upstream must not serve `newHeads`. A WebSocket URL is also required for subscriptions. See [Profile configuration](/configuration/profiles).
